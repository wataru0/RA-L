# 2021/04/19　Ours method program
# Curriculum-v4
# k=0からカリキュラムスタートさせるものとk=1からスタートするもの
# isis2020まとめ中の学習曲線の結果から着想を得た
# ADRとALLSTEPSの研究を参考にして，ドメインランダマイゼージョンとカリキュラムラーニングを合わせた手法提案ver1
# ALLSTEPS論文中のAdaptive curriculumを再現したい
# trainプログラム
# seedを指定して学習する

# ------実行コマンド！！！！-------
# python train_CDR.py --savedir=[保存するモデルの名前（dirになる（joint_rangeなどを書く！！）]
# ------------------------------
...
def mujoco_arg_parser():
    ...
    parser.add_argument('--savedir',help='saving name dir for trained mode!!!',type=str,default='Ant'+ datetime.datetime.now().isoformat()) #time使いたい
...
class ChangeJointRangeEnv(gym.Wrapper):
    ...
    def step(self,action):
        ...
        # actuator_map.append(action)
        # print(self.actuator_list)
        # print('action',action)
...
def main():
    ...
    plt.savefig(figdir + "/" + "parameter_fluctuation-" +str(args.savedir)+'-seed'+str(args.seed))
    ...
    np.savetxt(csvdir2+'/'+args.savedir+'-'+ 'joint_max-' +str(args.seed) +'.csv',joint_actuator_range_max,delimiter=',')
    np.savetxt(csvdir2+'/'+args.savedir+'-'+ 'joint_mix-' +str(args.seed) +'.csv',joint_actuator_range_min,delimiter=',')
